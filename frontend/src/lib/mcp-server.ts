...
// MCP Server configuration for ProjectKit context
...
export interface MCPTool {
  ...
  description: string;
  parameters?: any;
  execute: (params: any, context: ProjectKitContext) => Promise<any>;
}

class MCPServer {
  private context: ProjectKitContext = {};
  private tools: Map<string, MCPTool> = new Map();
  private conversationHistory: ChatMessage[] = [];
  private sessionId: string = this.getOrCreateSessionId();

  // Initialize MCP server with context
  initialize(context: ProjectKitContext = {}) {
  ...
  updateContext(updates: Partial<ProjectKitContext>) {
...
export function extractContextFromPath(pathname: string): Partial<ProjectKitContext> {
  const context: Partial<ProjectKitContext> = {};
